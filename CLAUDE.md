...
- **Read the recipe like it will fail.** Two failure classes have already cost builds: `dnf copr enable` cannot determine its chroot on a bootc base (use the COPR `.repo` file, which also templates `$releasever`), and vendor RPMs can be far larger than they look — check `rpm -qp --qf '%{SIZE}'` before layering something into every device's pull.
